...
| **PO.4 Define and Use Criteria for Software Security Checks** | `make ci` is the release gate: 95%+ coverage floor, critical modules at 100%, zero lint / security / anti-pattern / ASVS / docs-drift findings. |
...
| **PW.7 Test Executable Code to Identify Vulnerabilities and Verify Compliance with Security Requirements** | 889 unit + integration + hypothesis state-machine tests; 95% coverage with critical modules at 100%. Weekly OWASP ZAP Baseline on a live SKIFF. |
