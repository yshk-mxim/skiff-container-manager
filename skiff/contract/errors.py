...
_ERRORS: dict[str, _ErrorSpec] = {
    ...
    "system.debug_disabled": _ErrorSpec(
        403, "debug endpoint disabled — set SKIFF_DEBUG_THREADS=1 on the server to enable",
    ),
    ...
    "container.port_count_exceeds_cap":
        _ErrorSpec(400, "too many port mappings (max {limit})"),
    "container.port_host_privileged":
        _ErrorSpec(400, "host port {port} is privileged (< {threshold})"),
    "container.port_format":
        _ErrorSpec(400, "invalid port format"),
    "container.label_count_exceeds_cap":
        _ErrorSpec(400, "too many labels (max {limit})"),
    "container.label_bad":
        _ErrorSpec(400, "invalid label"),
    "container.command_too_long":
        _ErrorSpec(400, "command too long (max {limit} chars)"),
    "container.signal_bad":
        _ErrorSpec(400, "unsupported signal"),
    "container.volume_host_path_blocked":
        _ErrorSpec(400, "host path mounts are not allowed — use named volumes only"),
    "container.volume_format":
        _ErrorSpec(400, "invalid volume spec"),
    "container.update_no_fields":
        _ErrorSpec(400, "no updatable fields provided"),
    "container.memory_below_minimum":
        _ErrorSpec(400, "memory must be >= {minimum} bytes (Docker minimum)"),
    "container.memory_above_cap":
        _ErrorSpec(400, "memory exceeds cap of {cap}"),
    "container.cpu_above_cap":
        _ErrorSpec(400, "cpus exceeds cap of {cap}"),
    "container.cpu_shares_bad":
        _ErrorSpec(400, "cpu_shares must be an integer in [2, 1024]"),
    "container.pids_limit_bad":
        _ErrorSpec(400, "pids_limit must be an integer in [1, {cap}]"),
    "container.restart_policy_shape":
        _ErrorSpec(400, "restart_policy must be an object"),
    "container.restart_retry_bad":
        _ErrorSpec(400, "MaximumRetryCount must be an integer in [0, {cap}]"),
    "setup.scheme_bad":
        _ErrorSpec(400, "docker_host must use unix://, tcp://, or npipe:// scheme"),
    "setup.tcp_host_bad":
        _ErrorSpec(400, "tcp:// docker_host must specify an IP address, not a hostname"),
    "setup.tcp_port_bad":
        _ErrorSpec(400, "tcp:// docker_host must include a valid port"),
    "setup.token_too_short":
        _ErrorSpec(400, "api_token must be at least {minimum} characters"),
    "setup.docker_host_required":
        _ErrorSpec(400, "docker_host is required"),
    "setup.ssh_target_bad":
        _ErrorSpec(400, "ssh_target must be user@host"),
    "setup.env_managed":
        _ErrorSpec(403, "server is configured via environment variables — setup endpoint disabled"),
    "setup.already_done":
        _ErrorSpec(403, "already configured"),
    "setup.probe_disabled":
        _ErrorSpec(403, "probe endpoint disabled after setup completes"),
    "auth.token_unchanged":
        _ErrorSpec(400, "new_token is identical to the current token"),
    "auth.env_managed":
        _ErrorSpec(403, "API_TOKEN is managed via environment variable — update .env and restart to rotate"),
    "auth.reset_env_managed":
        _ErrorSpec(403, "cannot reset: server is configured via environment variables"),
    "ws.connections_exhausted":
        _ErrorSpec(429, "too many WebSocket connections from this IP"),
    "compose.project_dir_create_failed":
        _ErrorSpec(500, "failed to create project directory"),
    "compose.timeout":
        _ErrorSpec(504, "compose operation timed out"),
    "image.registry_search_failed":
        _ErrorSpec(502, "registry search failed"),
    "image.tag_fetch_failed":
        _ErrorSpec(502, "tag fetch failed"),
    "image.pull_timed_out":
        _ErrorSpec(504, "image pull timed out"),
    "image.push_timed_out":
        _ErrorSpec(504, "image push timed out"),
    "container.stats_timeout":
        _ErrorSpec(504, "stats call timed out"),
    "system.route_not_found":
        _ErrorSpec(404, "no route matches this path + method",
                   help="Check `docs/api-reference.md` or GET /api/openapi.json."),
    "system.method_not_allowed":
        _ErrorSpec(405, "this route does not accept that HTTP method",
                   help="Check the `Allow` header on the response for accepted methods."),
    "tunnel.not_configured":
        _ErrorSpec(404, "no managed tunnel configured",
                   help="The server has no stored SSH target. Run setup again via the wizard."),
    ...
    "tunnel.already_connected":
        _ErrorSpec(
            409, "Docker host is already reachable — no reconnect needed",
            help=(
    ...
    "auth.not_configured":
        _ErrorSpec(503, "server not configured — set API_TOKEN before accessing this endpoint"),
    "container.conflict":
        _ErrorSpec(409, "container conflict (already started/stopped?)"),
    "container.op_failed":
        _ErrorSpec(400, "container operation failed"),
    "container.bad_name":
        _ErrorSpec(400, "invalid container name (alphanumeric, dots, hyphens, underscores)"),
    "validation.bad_project_name":
        _ErrorSpec(400, "invalid project name"),
    "validation.bad_image_name":
        _ErrorSpec(400, "invalid image name format"),
    "validation.bad_mount_target":
        _ErrorSpec(400, "volume mount target must be an absolute path"),
    "validation.mount_target_blocked":
        _ErrorSpec(400, "mount target {path!r} is not permitted"),
    "validation.bad_tmpfs_shape":
        _ErrorSpec(400, "tmpfs must be an object mapping paths to options"),
    "validation.tmpfs_too_many":
        _ErrorSpec(400, "too many tmpfs mounts (max {max_mounts})"),
    "validation.tmpfs_bad_path":
        _ErrorSpec(400, "invalid tmpfs path"),
    "validation.tmpfs_path_blocked":
        _ErrorSpec(400, "tmpfs on {path!r} is not permitted"),
    "validation.tmpfs_bad_options":
        _ErrorSpec(400, "invalid tmpfs options"),
    "validation.tmpfs_size_exceeds_cap":
        _ErrorSpec(400, "total tmpfs size {total_mb:.0f}MB exceeds cap ({max_total_mb}MB)"),
    "compose.bad_services":
        _ErrorSpec(400, "invalid services section"),
    "compose.service_not_mapping":
        _ErrorSpec(400, "service '{svc_name}' must be a mapping"),
    "compose.service_forbidden_key":
        _ErrorSpec(400, "service '{svc_name}': '{key}' is not allowed for security reasons"),
    "compose.service_bad_network_mode":
        _ErrorSpec(400, "service '{svc_name}': network_mode '{net_mode}' is not allowed"),
    "compose.service_host_pid":
        _ErrorSpec(400, "service '{svc_name}': pid mode 'host' is not allowed"),
    "compose.service_bad_ipc":
        _ErrorSpec(400, "service '{svc_name}': ipc mode '{ipc_mode}' is not allowed"),
    "compose.service_host_volume":
        _ErrorSpec(400, "service '{svc_name}': host path mounts are not allowed"),
    ...
    "resource.not_found":
        _ErrorSpec(404, "resource not found"),
...
def http_error(
    ...
    **kwargs: Any,
    ...
    if extra:
        body.update(extra)
    return HTTPException(status_code=spec.status, detail=body)
