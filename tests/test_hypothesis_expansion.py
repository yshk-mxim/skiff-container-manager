...
class _UndoQueueMachine(RuleBasedStateMachine):
    ...
    @rule(kind=st.sampled_from(["container", "volume", "image", "network"]),
          rid=st.text(min_size=1, max_size=12))
    ...
    def fire_all_now(self) -> None:
        outstanding = {t for t, s in self.state.items()
                       if not s["cancelled"] and not s["fired"]}
        self.q.fire_all_now()
    ...
    def depth_consistent(self) -> None:
        outstanding = sum(1 for s in self.state.values()
                          if not s["cancelled"] and not s["fired"])
