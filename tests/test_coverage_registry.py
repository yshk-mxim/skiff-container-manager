...
def test_registry_search_success(client):
    ...
    with patch("skiff.routers.images.requests.get", return_value=mock_resp):
...
def test_registry_search_timeout_returns_502(client):
    with patch("skiff.routers.images.requests.get", side_effect=requests.exceptions.Timeout("timeout")):
        resp = client.get("/api/registry/search?q=nginx", headers=AUTH_HEADER)
    assert resp.status_code == 502


def test_registry_search_connection_error_returns_502(client):
    with patch("skiff.routers.images.requests.get", side_effect=requests.exceptions.ConnectionError("failed")):
        resp = client.get("/api/registry/search?q=nginx", headers=AUTH_HEADER)
    assert resp.status_code == 502
...
def test_registry_tags_official_image(client):
    ...
    mock_resp.raise_for_status = MagicMock()
    with patch("skiff.routers.images.requests.get", return_value=mock_resp) as mock_get:
...
def test_registry_tags_error_returns_502(client):
    with patch("skiff.routers.images.requests.get", side_effect=requests.exceptions.RequestException("error")):
        resp = client.get("/api/registry/tags?image=nginx", headers=AUTH_HEADER)
    assert resp.status_code == 502
